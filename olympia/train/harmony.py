...
class HarmonyModel():
			...
			if len(progression) > minLen:
				if len(set(progression)) >= 8:
					...
					for i in range(0, len(progression) - self.sequence_len, 1):
						...
						if set(prog_in) > 3: # we don't want series with only one note
					...
					if prog_input and prog_output:
						...
						categorical_input = np.reshape(categorical_input, (n_sequences, self.sequence_len, len(self.mapping)))
	...
	def train_harmony(self, epochs):
		...
		model.fit(np.array(self.inputs), np.array(self.outputs), epochs=epochs, batch_size=128, callbacks=[es])
	...
	def save_model(self, model):
		model_file = ROOT_DIR + '/djnn/files/models/harmony/model_' + self.model_name + '.json'
		weight_file = ROOT_DIR + '/djnn/files/models/harmony/weights_' + self.model_name + '.h5'
