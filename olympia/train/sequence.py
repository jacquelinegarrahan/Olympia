...
class SequenceModel():
	...
	def prepare_sequences(self):
		...
		for sequence in self.sequences:
			...
			for i in range(0, len(sequence) - self.sequence_len, 1):
				...
				prog_inputs.append(prog_in)
				prog_outputs.append(prog_out)
			
			n_sequences = len(prog_inputs)
			categorical_input = np_utils.to_categorical(inputs, num_classes=self.n_clusters)
			categorical_input = np.reshape(inputs, (n_sequences, self.sequence_len, self.n_clusters))
			categorical_output = np_utils.to_categorical(outputs, num_classes=self.n_clusters)
			categorical_input = inputs / self.n_clusters
	...
	def save_model(self, model):
		model_file = ROOT_DIR + '/djnn/files/models/sequence/model_' + self.model_name + '.json'
		weight_file = ROOT_DIR + '/djnn/files/models/sequence/weights_' + self.model_name + '.h5'
...
if __name__ == '__main__':
	...
	sequence_model = SequenceModel(song_objs, n_clusters, n_measures, sequence_len, midi_dir, model_name)
	sequence_model.train_sequences(50)
