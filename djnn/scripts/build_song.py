...
def writeMidi(generatedHarmony, generatedDurations, key, songName):
	...
	for i, nt in enumerate(generatedHarmony):
		...
		if i == 0:
			...
			genNote = note.Note()
			genNote.pitch = getBestNote(rn.pitches, outputChords[-1])
	...
	#	new_chord = chord.Chord(rn.pitches)
		...
		outputChords.append(rn.pitches)
...
def getBestNote(pitches, lastPitches):
	newChordPS = [pitch.ps for pitch in pitches]
	oldChordPS = [pitch.ps for pitch in lastPitches]

	difIndices = []
	for i in len(newChordPS):
		if newChordPS[i] != oldChordPS[i]:
			difIndices.append(i)
	...
	if len(difIndices) == 0:
		...
		return pitches[numpy.random.choice(difIndices)]
